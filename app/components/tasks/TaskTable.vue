...
interface Props {
  ...
  displayMode?: 'flat' | 'hierarchical' // Display mode for tasks
}
...
// Expanded state for hierarchical view
const expandedTasks = ref<Set<string>>(new Set())
const loadingSubtasks = ref<Set<string>>(new Set())

// Toggle task expansion in hierarchical mode
async function toggleExpand(task: Task, event: MouseEvent) {
  ...
  if (expandedTasks.value.has(task.id)) {
    ...
    if (task.subtaskCount > 0 && !task.subtasks) {
      loadingSubtasks.value.add(task.id)
      loadingSubtasks.value = new Set(loadingSubtasks.value)
...
// Clear loading state when subtasks are loaded
watch(() => props.tasks, () => {
  // Clear loading state for tasks that now have subtasks
  props.tasks.forEach(task => {
    if (task.subtasks && loadingSubtasks.value.has(task.id)) {
      loadingSubtasks.value.delete(task.id)
    }
  })
  loadingSubtasks.value = new Set(loadingSubtasks.value)
}, { deep: true })

// Build flat list of tasks with subtasks for rendering (in hierarchical mode)
const flattenedTasks = computed(() => {
  if (props.displayMode === 'flat') {
    return sortedTasks.value.map(task => ({ task, depth: 0 }))
  }
  ...
  function addTaskAndChildren(task: Task, depth: number) {
    ...
    if (expandedTasks.value.has(task.id) && task.subtasks) {
...
<template>
  ...
  <template v-else>
    ...
    <div class="hidden lg:block">
      <table class="w-full min-w-full border-collapse" aria-label="Tasks list">
        ...
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
          ...
          <template v-else>
            <tr
              v-for="({ task, depth }, index) in flattenedTasks"
              :key="task.id"
            ...
            >
              ...
              <td class="px-3 py-2 whitespace-nowrap">
                <div class="flex items-center gap-2" :style="{ paddingLeft: `${depth * 20}px` }">
                  <!-- Expand/collapse button for hierarchical mode -->
                  <button
                    v-if="displayMode === 'hierarchical' && task.subtaskCount > 0"
                    class="flex-shrink-0 w-5 h-5 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition-colors"
                    :title="expandedTasks.has(task.id) ? 'Collapse subtasks' : 'Expand subtasks'"
                    @click="toggleExpand(task, $event)"
                  >
                    <svg
                      v-if="loadingSubtasks.has(task.id)"
                  ...
                  <!-- Spacer when no expand button but in hierarchical mode -->
                  <div v-else-if="displayMode === 'hierarchical'" class="w-5 flex-shrink-0" />
                  <div
                    v-if="enableDragDrop"
    ...
    <div class="lg:hidden space-y-2">
      ...
      <template v-else>
        <div
          v-for="({ task, depth }) in flattenedTasks"
          :key="task.id"
          ...
          :class="{ 'ml-4 border-l-2 border-l-primary-200 dark:border-l-primary-800': depth > 0 }"
        ...
        >
          ...
          <div class="flex items-start gap-3 mb-3">
            <!-- Expand/collapse button for hierarchical mode on mobile -->
            <button
              v-if="displayMode === 'hierarchical' && task.subtaskCount > 0"
              class="flex-shrink-0 w-6 h-6 flex items-center justify-center text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition-colors mt-0.5"
              :title="expandedTasks.has(task.id) ? 'Collapse subtasks' : 'Expand subtasks'"
              @click.stop="toggleExpand(task, $event)"
            >
              <svg
                v-if="loadingSubtasks.has(task.id)"
